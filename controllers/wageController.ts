...
const wageController = {
  sendWage: async (ctx: Context) => {
    let { officer_id, total_wage, uid } = ctx.request.body;
    if (!(officer_id && total_wage && uid)) {
    ...
    try {
      ...
      let oldWage = await wageDao.getWageByUid(uid, year, month);
      if (oldWage) {
  ...
  getWageById: async (ctx: Context) => {
    let { uid, year, month } = ctx.request.query;
    if (!uid) {
    ...
    try {
      let wage: Wage = await wageDao.getWageByUid(Number(uid), Number(year), Number(month));
  ...
  getWagesByMonth: async (ctx: Context) => {
    let { year, month } = ctx.request.query;
    ...
    try {
      let wages = await wageDao.getWagesByMonth(Number(year), Number(month));
